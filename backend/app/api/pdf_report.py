...
def _add_page_bg(canvas, doc):
    ...
    canvas.drawCentredString(
        A4[0] / 2, 10 * mm,
        f"QDS-SIEM  •  Cyber Threat Detection  •  Generated {datetime.utcnow().strftime('%Y-%m-%d %H:%M UTC')}  •  Page {doc.page}"
    )
    canvas.restoreState()
...
async def generate_pdf_report(
    ...
    story.append(Paragraph(
        "End of Report — QDS-SIEM Cyber Threat Detection Framework",
        styles["FooterStyle"]
    ))
